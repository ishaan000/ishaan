...
const SpaceBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkDevicePerformance = () => {
      ...
      const hasSlowProcessor = navigator.hardwareConcurrency
    ...
    const maxShootingStars = isLowPerfDevice
      ? 5
      : window.innerWidth < 768
      ? 8
      : 15; // Fewer on mobile/low-perf
    ...
    const animate = (timestamp: number) => {
      ...
      stars.forEach((star) => {
        ...
        // Twinkle effect
        star.opacity =
          0.5 + (Math.sin(timestamp * 0.001 * star.speed) + 1) * 0.25;
      });
      ...
      if (
        timestamp - lastShootingStarTime > shootingStarInterval &&
        Math.random() < shootingStarProbability
      ) {
        createShootingStar();
